    ...
    public CustomerModel customerEntityToModel(Optional<CustomerEntity> customerEntity) {
        ...
        List<AccountModel> accounts = customerEntity.orElse(new CustomerEntity())
                ...
                .map(accountEntity -> {
                            ...
                            return new AccountModel(accountNumber,accountEnum,amount
                                    ,creationTime,lastUpdatedTime,null);
                        }
                ).collect(Collectors.toList());

        String customerId = customerEntity
                .orElseThrow(() -> new AccountException(ExceptionMessageEnum
                        .CUSTOMER_NOT_FOUND.name())).getCustomerId();

        return new CustomerModel(customerId
                , customerEntity.get().getFirstName()
                , customerEntity.get().getLastName()
                , customerEntity.get().getMiddleName(),accounts);
